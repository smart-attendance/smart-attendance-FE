...
function AttendList({schedules, showing, schedule}) {
    ...
    function handleDelete(id) {
        ...
        if (confirm) {
            setIsloading(true);
            axios.delete(`https://smart-attendance-be.herokuapp.com/api/schedule/${id}`, {headers: {"Authorization": "Bearer " + token}})
            .then(res => {
                            ...
                            <tr key={id}>
                                ...
                                {item.id ? (
                                    <React.Fragment>
                                        <td className="px-4">
                                            <center>
                                                <button className="bg-green-primary hover:bg-blue-300 rounded p-1 m-1" onClick={() => { showing(3); schedule(item) }} >
